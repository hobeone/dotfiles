...
0. **Resolve the active rule set.**
   ...
   1. Check the project's `GEMINI.md` (or equivalent contributor / agent-guidance file) for a `Language:` declaration line (e.g., `Language: cpp`). If present, load `quality-list/lang-<value>.md`.
...
2. **Process every item in preflight mode.** Read `quality-list/SKILL.md`'s Items index using `view_file` to get the authoritative item set, and process every item in it. Use the per-item Preflight framing quick reference below as the compressed lens — an accelerator over that index spine, not the authoritative set. When an index item has no quick-reference entry, `view_file` its `quality-list/items/<slug>.md` as a safe fallback, so a missing entry never silently drops the item. Read the index for the item set (the slugs); do **not** read the item bodies into main context up front. Only `view_file` a specific `quality-list/items/<slug>.md` file when (a) the item is absent from the quick reference, (b) the quick reference is insufficient to decide active vs N/A, or (c) the item is △ active and the setup action needs the full concern conditions / N/A criteria.
...
- **`paired-artifact-drift`** — List every textual surface that will need updating: same-crate `grep_search`/`code_search` targets, parent module docstrings, `examples/`, `bench/`, doctests, `README.md`, `GEMINI.md`, migrations / schemas, declared cross-repo paired artifacts. Plan the sweep before editing primary identifiers.
