...
description: Work a GitHub issue end to end in two phases — research, then implement — under research and implement discipline.
---
...
Before research begins, settle the workspace and branch via `run_command`:
...
3. **Decision gate**:
   ...
   - Already in a worktree or on a non-default branch → treat it as the intended workspace and proceed. Only stop if the branch name plainly contradicts the issue (e.g., on `feat/100-foo` while working #200) — in that case announce the mismatch and ask via `ask_question`.
...
The plan posted to the issue or artifact MUST include the `Inconclusive / Deferred items` section (or `Inconclusive / Deferred items: none identified`). This section is the discovery contract Phase 2 will enforce.
