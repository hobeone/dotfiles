...
Before research begins, settle the working branch via `run_command`:
...
3. **Decision gate**:
   ...
   - On a non-default branch → treat it as the intended branch and proceed. Only stop if the branch name plainly contradicts the issue (e.g., on `feat/100-foo` while working #200) — in that case announce the mismatch and ask via `ask_question`.
