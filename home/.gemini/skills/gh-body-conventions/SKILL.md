...
Write the body to a file (typically under `/tmp/`) and pass it via `--body-file` (using `gh-post` if available, or `gh` directly). Never pass the body inline to `gh` (`gh (issue|pr) (create|edit|comment) --body ...`); always use stream/file input (`--body-file`). Writing the body file itself with a heredoc (`cat > /tmp/body.md <<'EOF' ... EOF`) or via `write_to_file` is fine.
...
- When linking to specific files or code lines in markdown documents or comments, use the `file://` link format: `[filename](file:///path/to/file#L10-L20)`.
...
- **Internal tooling citing itself as evidence.** A claim backed by an unpublished local script, a subagent/review-pass dispatch, or any verification step with no public GitHub trace (issue, PR, commit, CI run) — even when the underlying claim is true. Test each citation: does it resolve to something an external reader can open, or only to a process that ran in this session? ("A prior escalated code-review pass flagged this" and "verified via a standalone reproduction" are both this shape — the fix is either linking a real public artifact or dropping the appeal to authority and letting the claim stand on its own stated reasoning.)
