...
GitHub-integrated execution skill. Drives the Plan → Do → Study → Act workflow with plan-vs-actual drift surfacing and a final `done-check` audit.

**Issue / task:** $ARGUMENTS

## Step 1 — Retrieve the plan

1. Read the GitHub issue and comments using `run_command`: `gh issue view $ARGUMENTS --comments`.
...
Follow the Plan → Do → Study → Act workflow with scoped units, fresh verification evidence, diff review, fix loops, and commit/push discipline. The wrapper layers the following additional rules on top.

### 3.0 Preflight via todo-check

Before any code change, invoke `todo-check` against the plan to extract the active `quality-list` items and their setup actions for this work. Hand the resulting △ rows to the Plan step so the unit checks already include them. Re-invoke `todo-check` between units when the next unit changes the active item set (e.g., it introduces a new public API → `impact-verification` and `paired-artifact-drift` become active).

### 3.0.1 Pre-commit hook recall

Once per session (cache for the duration of the conversation), read the project's pre-commit configuration to identify the constraints each hook will enforce using `view_file` or `find_by_name`. Typical config locations: `.pre-commit-config.yaml`, `lefthook.yml`, `.husky/`, the `[hooks]` section of a project task runner (e.g. `Makefile.toml` / `Justfile`).
...
1. Read `MEMORY.md` (index only) via `view_file`.
2. For each entry whose one-line description plausibly intersects a unit in the Step 2 checklist, open and read it with `view_file`.
...
Once per session, read any contributor / agent guidance docs present at the repo root or in `docs/` (top-level only) via `view_file`: `GEMINI.md`, `CLAUDE.md`, `AGENTS.md`, `CONTRIBUTING.md`, `STYLE.md` / `STYLEGUIDE.md`, `HACKING.md`, `docs/CODING_GUIDELINES.md`, `docs/conventions/`. `README.md` only if it carries contribution guidance. Treat clearly-labeled non-standard names as in-scope. Thread the constraints into per-unit checks the same way as 3.0.2 — violations are discovery-contract violations.

### 3.1 Baseline

Before any code change, build and run existing tests via `run_command` to record pre-existing failures, and run the project's linter (e.g., `go test`, `cargo clippy`, `clang-tidy`, `ruff check`, `eslint`) to record pre-existing warnings. Compare against both baselines at every verification step. Pre-commit-gated linters (Step 3.0.1) are especially important — the gate rejects on any new violation. Formatters are excluded.
...
2. **Check the `Deferred` items.** Is this discovery the resolution-point of a deferred concern?
   - **Yes** → surface to user via `ask_question`, decide whether to expand scope or keep it deferred.
...
2. **Verify the substitute preserves each property.** Any dropped property must be named in the plan-vs-actual diff and surfaced to the user via `ask_question` — "substituting M with M' to avoid unsafe; M' loses Q. OK to ship?" — not silently discarded.

### 3.3 Quality items during Do / Study

Substantive rules for guards, fixtures, docstring consistency, textual drift, naming-as-claim, and plan-vs-actual reconciliation live in `quality-list`. Honor whichever items Step 3.0's `todo-check` marked active for the current unit. The wrapper-specific gates (3.2 / 3.2.1 / 3.2.2 above) layer on top. Apply file edits using `replace_file_content` or `multi_replace_file_content`. Write new files with `write_to_file`.
...
2. **Commit message(s)**: inherit and strictly follow the project's or agent's configured commit message conventions (e.g. from `GEMINI.md`, `CLAUDE.md`, or user rules).
