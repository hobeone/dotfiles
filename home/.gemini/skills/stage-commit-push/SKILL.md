...
One-shot skill for the review-fix loop: stage modified files, generate a commit message, commit, and push using `run_command`.
...
Inspect the staged diff and recent commits to produce a commit message that inherits and strictly follows the project's or agent's configured commit conventions (e.g. from `GEMINI.md`, `CLAUDE.md`, or user rules).
...
<commit message formatted per project/agent conventions>
EOF
)"
```
