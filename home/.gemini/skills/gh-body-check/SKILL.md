...
Write the body to a temp file using `write_to_file` or bash heredoc via `run_command`:
...
```bash
~/.gemini/skills/gh-body-check/body-math-scan.sh "$BODY_FILE"
```
...
Invoke `invoke_subagent` with `TypeName: "self"` and `Role: "Cold-Reader Audit"`. Pass only the body, the target repo name, and the artifact kind. Do NOT pass chat history, the plan, the author's prior messages, or any context about why the body is being filed — the fresh context is the entire point.
