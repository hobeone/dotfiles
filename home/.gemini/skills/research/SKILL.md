...
GitHub-integrated research skill. Drives the Finding → Hypothesis → Defense → Probe → Decision workflow with parallel subagents (`invoke_subagent`) and posts the resulting plan to GitHub or session artifact. Do NOT write any production code in this skill.
...
- **Number** → `gh issue view $ARGUMENTS` (via `run_command`) and proceed with that issue.
...
1. If existing issue, read it with `gh issue view` via `run_command`. Otherwise use the free text.

2. Skim the project layout (`find_by_name`, `list_dir`, `GEMINI.md`, key entry points). Do NOT deep-read files yet.

3. **Establish baselines.**

   - **Test baseline**: build and run existing tests using `run_command`. Record pre-existing failures so later regressions can be distinguished.
   - **Memory recall baseline**: read `MEMORY.md` (index only); for each entry whose one-line description plausibly intersects the work, open and read it with `view_file`. A hypothesis that contradicts an active memory rule is rejected at formation.
...
5. Each hypothesis is phrased as a **falsifiable Review Claim or Hypothesis** — concrete enough that a subagent can attempt to disprove it.

6. **Tag each hypothesis with a `kind`:**

   - `empirical` — resolved by reading code (`view_file`), running tests (`run_command`), observing runtime, inspecting git history, querying spec / external API / caller behavior. Subagent probes in Step 2 handle these.
...
8. **Present hypotheses to the user for approval before spawning subagents.** Use `ask_question` when presenting options. Show the `kind` tag for each. The user can narrow scope, split into sub-issues, or correct mis-classifications.
...
### Step 2.A — Empirical hypotheses (subagents via `invoke_subagent`)

Spawn one subagent per `empirical` hypothesis (or per small related group) using `invoke_subagent` with `TypeName: "self"` (or `TypeName: "research"` / `TypeName: "research-google"`).

Subagent contract:

1. **Assigned hypothesis**: set as the initial Review Claim or Hypothesis.
2. **Probes are three-way**: Supporting / Disconfirming / Scope. The Disconfirming probe **must be actually executed**, not just listed.
3. **For non-trivial existing code, read deeply** using `view_file`, `code_search`, `grep_search`.
4. **Runtime probe (preferred for behavioral claims).** Behavioral claims (output ordering, return shape, error-path return, ABI / FFI layout, signal handling, performance) need a minimal reproducer via `run_command` — docs reading is corroboration, not verification. Boundary cases (minimal/maximal sizes, type variations, single-element containers) belong here.
...
6. Return a **Decision** in the four-state shape (`confirmed` / `rejected` / `inconclusive` / `deferred`). For `inconclusive`, attach the remaining `probe:`. For `deferred`, attach reason and resolution-point.
7. Report back with concise file paths, line numbers, function signatures, and probe results.

**Subagent granularity**: hypotheses requiring only existence checks or single-file grep can be resolved directly from the main context using `view_file` or `grep_search`. Reserve subagents for hypotheses that touch multiple files or need deep reading. Spawn all empirical-hypothesis subagents in a single `invoke_subagent` call.
...
1. **Workspace patterns** — search the workspace (`code_search`, `find_by_name`) for analogous constructs. If the workspace consistently does X, that is the answer.
...
A sub-decision is genuinely unresolved only when the above are silent or contradictory. "More than one technically-viable option exists" is not unresolved — it is analysis you owe the user. When you do escalate, state what you checked via `ask_question`.

Report the plan back to the main context. Write the plan to session artifact `<appDataDir>/brain/<conversation-id>/YYYY-MM-DD-<feature-name>.md` with `ArtifactMetadata` (`RequestFeedback = true`, `UserFacing = true`).

## Step 3.4 — Reachability check (mandatory)

A plan can be locally closed yet reach further than the author reasoned about — a new public surface whose semantics depend on consumer code outside scope (Checks 1–3), an enablement that binds compilation units the plan never named (Check 4), a verification probe that covers fewer build configurations than the obligation spans (Check 5), or a performance probe that exercises fewer code paths than the changed path it is trusted to measure (Check 6). Author confidence alone misses these — they require looking outward, deliberately, at what the plan didn't name. Any check firing means the plan's reach is not closed; rescope or defer.
...
Grep the plan body and proposed docstrings (case-insensitive) via `grep_search`:
...
3. **Loop gate**: after patching, re-run the review pass fresh (no bias from the previous iteration's findings). Exit the loop when a pass raises no new concerns. Cap: 3 iterations within the same premise. If the cap is reached, or a pass keeps raising the same unresolved concern, surface the outstanding findings to the user via `ask_question` and ask whether to proceed as-is, patch further manually, or escalate. Premise concerns return to Step 1 and reset the counter — iteration count is per-premise, not lifetime.
...
Present the plan (after any Step 3.5 revisions) and ask for approval via `ask_question` before posting to GitHub.
...
- **Existing single-scope issue** → `gh-post issue comment $ARGUMENTS` (or `gh issue comment $ARGUMENTS --body-file` fallback per `file-issue` step 5) with the plan as body.
...
- **New task** → `gh-post issue create` (or `gh issue create` fallback per `file-issue` step 5) with the plan in the body; report the new issue number.

When ambiguous, ask the user via `ask_question`. Umbrella sub-issue is the D1 default.
