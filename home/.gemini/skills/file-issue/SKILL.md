...
Draft a GitHub issue body that follows the user's conventions, show the draft for approval, then file via `gh` (using `run_command`).
...
- **First body line.** `Parent: #<umbrella#>`. Always included so tooling can detect the umbrella linkage.
...
- **Frozen-contract discipline.** The sub-issue body is written at file time and not edited during implementation. Drift discovered during implementation goes to the PR's `Plan-vs-actual delta` (and, when a reader genuinely needs it, a `Notes` caveat in the PR body framed as the code's own limitation), never back to the issue body. Editing the body rewrites history that PR titles, commit messages, and `Closes #N` references already point to.
...
- Target repo (`run_command`: `gh repo view` if unclear).
...
Write the laundered body to a temp file (via `write_to_file`), then invoke the wrapper via `run_command`:
...
`gh-post` is a single-entry wrapper that funnels every body through stream input (`--body-file` or `--body-stdin`) and re-runs the hardwrap validator before forwarding to `gh`. Always file the issue through `gh-post` if available. If `gh-post` is missing, fall back to `gh issue create --repo <owner>/<repo> --title "<title>" --body-file /tmp/<descriptive-name>.md`.
