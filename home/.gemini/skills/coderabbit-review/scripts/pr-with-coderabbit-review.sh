...
# Routes through `gh-post pr create` (or `gh pr create` fallback) so the body
# passes submission. No reviewer flag: the CodeRabbit app starts its review on PR open by itself.
echo "Creating PR (CodeRabbit reviews automatically on open)..." >&2
if command -v gh-post &>/dev/null; then
