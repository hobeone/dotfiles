...
1. `/code-review` cannot be invoked programmatically from within a pipeline skill (`disable-model-invocation` — the Skill tool always rejects it). Use the `pr-review-toolkit:code-reviewer` agent instead, at an effort matching `medium` (raise to a more thorough pass for large or risky diffs; keep the chosen effort fixed across this PR's iterations). Do not attempt `Skill: code-review` first — it will always fail and the failure carries no new information.
   ...
   **When the diff changes what flows into an existing, unmodified function** — a context that now carries a deadline it never used to, a flag that can now be true, a channel that's now buffered — include this in the dispatch: ask the reviewer to read that unmodified function and re-derive its concurrency/timing assumptions under the new input, not just check that the touched lines are correct in isolation. A diff-scoped review otherwise never asks "does this activate a previously-dormant code path or race window in code the diff never touched" — the failure mode that let a real deadline-poisoning race in `setupHandshakeDeadline` (#488/#498, gonzbd) ship past this exact gate: the diff only wrapped the context passed in, never modified the function whose timing profile that wrap changed.
...
7. **Final claim sweep — once the loop has converged, before the merge gate.** Run `pr-review-toolkit:comment-analyzer` over the *cumulative* PR diff, and separately `git grep` the distinctive phrasing of every claim the change falsified, **from the repository root** — not a path list. `git grep` searches tracked files only, so generated and vendored trees drop out without enumerating them, and it cannot miss a directory nobody thought to include. Restricting the search to the directories you were already thinking about is the failure mode, not a shortcut: the first version of this step said "the source tree and the topic docs" and a sweep written that way missed a contradicting copy in a sibling doc. Fix what it finds through `/stage-commit-push` as usual, then stop — do not re-enter the fix loop for prose unless the sweep found something that changes behaviour.

   The two halves cover different things and neither substitutes: the analyzer reads the comments the PR *changed*, the grep finds the ones it *didn't*.

   This runs **last**, not per-round: each round's own fix creates fresh drift, so an early sweep goes stale and a per-round sweep is mostly re-checking prose that is about to change again. It is a separate step from the `paired-artifact-drift` row in `/done-check` — see Rules for why that row does not cover this.
...
- **A test added in the fix loop is not a pin until it has been observed red.** A reviewer finding of the form "this fix has no test" is answered by a test that *fails without the fix*, and pins written under review pressure have shipped green against unfixed code more than once. Revert the specific fix — each half separately, if it has two — run the new test, confirm it fails and that the message names the right defect, then restore. Never via `git stash`: the stash stack is shared across sessions and popping can take another session's work. Copy the file aside (`SCRATCH="$(mktemp -d)"; trap 'rm -rf "$SCRATCH"' EXIT`), mutate in place, restore from that copy — not `git checkout -- <path>`, which also discards unrelated uncommitted edits in the file, the same hazard as the shared stash for the same reason. Prefer neutering a condition over deleting a block, since deleting tends to break the build instead of the test, and a compile error does not show the test would have caught anything. Quote the observed failure in the reply to the finding — a red-green claim without the message it produced is an assertion.
...
- **Done-check delta mode.** Report only rows whose status changes from the previous audit, plus any new ⚠. Resolve every new ⚠ before the subsequent `/stage-commit-push`. Pay special attention to:
  ...
  - `paired-artifact-drift`: every comment / docstring / PR-body sentence touched by or referring to the fixed code must still be accurate. **This row is scoped to the fix's own neighbourhood and that is not enough on its own** — a behaviour change falsifies the same sentence in places the diff never touches: an interface doc, a sibling field's comment, a topic doc, a migration's comment block. Correcting the copy you were editing leaves the others reading as authoritative. That is what Phase 1 step 7's sweep is for; this row does not replace it, and a green `paired-artifact-drift` is not evidence the claim is dead everywhere. Grep the *claim*, not the changed files.
