...
- `Review completed` + no review object → **ambiguous**: either a genuine zero-finding pass or a **non-review skip** that still emits terminal `success`. A skip means the review never ran, so it must never be recorded as zero findings. Four skip causes are known; the script rules them all out (exit `2`) before declaring a clean pass.
  - **Announced in the commit status** — the status description itself reads `Review skipped: …` and names its own cause (observed: `manual review required for this OSS repository`, on a repo whose plan or settings exclude automatic review). Checked **first**, and the only cause CodeRabbit states outright rather than leaving to be inferred. It emits **no bot-comment marker at all**, so the three marker checks below cannot see it — a run carrying it was reported as a clean pass on three separate PRs before the cause was found. `@coderabbitai review` is worth trying but does not always take; where the plan or settings are the cause, it will not, and the honest record is "not reviewed" rather than a nudge repeated until something changes.

  The remaining three announce nothing and are inferred from discriminators in CodeRabbit's bot issue comments:
