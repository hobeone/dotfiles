...
grep -c "premise concern" home/.claude/skills/research/SKILL.md      # expect >= 3
grep -n "Cap: 3 iterations" home/.claude/skills/research/SKILL.md    # must still be present
```

Expected: `1`; a count of at least 3; and the iteration cap line intact. The lens pass reuses the existing loop gate and cap — it must not have introduced a second one.
